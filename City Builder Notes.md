...
- When i will place a bakery the Builder will Build it and 1 bakery need 2 NPC chef for the Bakery Job Create a new Job "Chef" in the Job Priority Panel 1 bakery will have 2 chef and they will create 2 bread in 5 second means 1 bread each. also create a variable for Total Number of Breads. Each Bakery will have its own total number of bread count the Bakery Panel will show the Total count in the UI Panel the Current bakery Building is holding(2 NPC will go inside the Bakery and stay there playing the IsDoing animation don't hide the NPC sprite).Also Create a Delivery Job and their work will be to take the Bread from the Bakery and deliver it to all the Required houses each shop can have 2 Delivery NPC this will help us to find the requirement of the delivery guy and the chef(this will be handled by the same carrying animation).  
